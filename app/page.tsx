...
export default function Home() {
  ...
  const [hoveredImage, setHoveredImage] = useState<{ generationId: string; imageIndex: number } | null>(null)
  const [isPlaying, setIsPlaying] = useState<{ [key: string]: boolean }>({})
  const [showModelSelection, setShowModelSelection] = useState(false)
  const [isTransitioning, setIsTransitioning] = useState(false)
  const [loadingGeneration, setLoadingGeneration] = useState<{ id: string; prompt: string } | null>(null)
  ...
  const models: {
    ...
    },
    {
      ...
      imageUrl: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Z.jpg-dpvX4dqgGQks8OOB8NZVQZUTMAcTnh.jpeg",
      category: "styles",
    },
    {
      id: "sketch",
  ...
  const extractTwoUrlsAndPrompt = (data: any): { imageUrls: string[]; enhancedPrompt: string | null } => {
    let imageUrls: string[] = []
    let enhancedPrompt: string | null = null
      ...
      if (data && typeof data === "object" && !Array.isArray(data)) {
        ...
        if (imageUrls.length === 0) {
          ...
          for (const [key1, key2] of urlPairs) {
            if (data[key1] && data[key2] && typeof data[key1] === "string" && typeof data[key2] === "string") {
  ...
  const handleGenerate = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        let webhookUrl = "https://humanprompt.app.n8n.cloud/webhook/eff83ff9-6f5e-4786-9a2c-3d15ca5fc4ed" // Default webhook for no style/character

        if (modelsEnabled) {
          if (selectedModel === "fiona") {
            webhookUrl = "https://humanprompt.app.n8n.cloud/webhook/fd13245b-3136-40de-82cc-8d220a3758c4"
          } else if (selectedModel === "camilla") {
            webhookUrl = "https://humanprompt.app.n8n.cloud/webhook/9cd5b4aa-96a3-44d7-afdb-3f5f76cea56a"
          } else if (selectedModel === "cinematic") {
            webhookUrl = "https://humanprompt.app.n8n.cloud/webhook/09b186fb-af85-4632-bed9-88381b766b51"
          } else if (selectedModel === "sketch") {
            webhookUrl = "https://humanprompt.app.n8n.cloud/webhook/b92deb2e-a2af-4c7e-a59e-aba30d7f1a8b"
          }
        }
        ...
        if (response.ok) {
          ...
          try {
            ...
            logInfo(enhancedPrompt ? "Extracted enhanced prompt from response" : "No enhanced prompt found in response")
    ...
    try {
      ...
      if (mode === "advanced") {
        try {
          ...
          webhookUrl = "https://humanprompt.app.n8n.cloud/webhook/a64ecae2-2790-4f39-9134-07c20e0bf6a8"
          ...
          if (response.ok) {
            ...
            setDebugInfo((prev) => ({
              ...
              webhookResponses: [...(prev?.webhookResponses || []), { type: "advanced_init", response: responseText }],
            }))
            ...
            try {
              ...
              console.log(`Received request_id: ${parsedData.request_id} for advanced video generation`)
      ...
      try {
        ...
        if (response.ok) {
          ...
          setDebugInfo((prev) => ({
            ...
            webhookResponses: [...(prev?.webhookResponses || []), { type: `${mode}_video`, response: responseText }],
          }))
          ...
          console.error(`${mode} mode: Failed to send image URL for video generation. Status: ${response.status}`)
        ...
        <div className="bg-[#101218] rounded-lg p-8 max-w-5xl w-full">
          ...
          <div className="grid grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <p className="text-white text-lg">
                    quality: may include visual artefacts or imperfections; ideal for rapid concept testing
                  </p>
                </div>
              </div>
            ...
            <div className="bg-[#1E1E1E] rounded-lg p-6">
              <div className="space-y-6">
                <div>
                  <p className="text-white text-lg">speed: generates videos in approximately 7 minutes</p>
                </div>
                <div>
                  <p className="text-white text-lg">
                    quality: fewer visual artefacts, offering higher accuracy and professional-quality outputs
                  </p>
                </div>
              </div>
              <div className="mt-8 flex justify-center">
  ...
  const openFullscreenMedia = (type: "image" | "video", url: string, generationId?: string, videoId?: number) => {
  ...
  const toggleFullscreenVideo = () => {
    if (!fullscreenMedia?.type === "video" || !fullscreenMedia.generationId || !fullscreenVideoRef.current) return
              ...
              <div className="space-y-2">
                {debugInfo.webhookResponses.map((response, index) => (
                  <div key={index} className="bg-gray-800 p-3 rounded">
                    <h4 className="font-semibold mb-1">{response.type || `Response ${index + 1}`}:</h4>
  ...
  const renderGeneration = (generation: Generation) => (
    <div key={generation.id} className="w-full flex flex-col items-end mb-16 px-4 animate-chat-message">
      {/* Prompt display */}
      <div className="mt-8 text-right">
        ...
        <div className="text-gray-400 text-sm mt-1">
          {generation.enhancedPrompt ? "click to see enhanced prompt" : "no enhanced prompt available"}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {generation.imageUrls.map((url, index) => (
            <div
              ...
              onMouseEnter={() => setHoveredImage({ generationId: generation.id, imageIndex: index })}
      ...
      {/* Click instruction */}
      <div className="text-right text-white mt-4 mb-8">click the image you'd like to turn into a video</div>
                ...
                {video.error ? (
                  ...
                  <div className="aspect-video relative overflow-hidden rounded-lg cursor-pointer">
                    <video
                      ...
                      loop
                      playsInline
                      onClick={() => openFullscreenMedia("video", video.url, generation.id, videoIndex)}
                    />
  ...
  const renderFullscreenMedia = () => {
    ...
    return (
      ...
      >
        <button className="absolute top-4 right-4 text-white hover:text-gray-300 z-10" onClick={closeFullscreenMedia}>
          <X size={32} />
        </button>
        ...
        <div className="flex flex-col min-h-screen">
          ...
          <div className="flex-grow flex flex-col">
            <div className="flex-grow">
              {/* Show loading skeletons when transitioning or loading */}
              {isTransitioning || (isLoading && loadingGeneration) ? renderLoadingPlaceholders() : null}
            </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 px-4">
        ...
        <div className="text-center mb-2">
          <Image src="/images/logo-white-alpha.png" alt="Human Prompt" width={700} height={124} className="h-auto" />
        </div>
